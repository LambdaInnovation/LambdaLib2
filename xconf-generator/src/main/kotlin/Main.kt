...
object Main {
    ...
    class ImportItemConfig(val file: String, val classPath: String, val classAnnotations: Array<String>)
    ...
    class ImportConfig(
        ...
        val itemConfigs: List<ImportItemConfig>,
        val blockConfigs: List<ImportItemConfig>,
    ...
    private fun writeItems(ctx: BaseContext) {
        ctx.config.itemConfigs.forEach {
            val rawItems = ConfigFactory.parseFile(File(ctx.rootDir, it.file))
            val itemBase = rawItems.getConfigOrNull("_base")
            val items = rawItems
                .root()
                .filter { !it.key.startsWith("_") }
                .map { (id, elem) ->
                    val obj = (elem as ConfigObject).toConfig().let { x ->
                        if (itemBase != null)  x.withFallback(itemBase) else x
                    }
                    ItemMetadata(
                        id = id,
            ...
            run {
                val itemClassFile = File(ctx.srcDir, it.classPath.replace('.', '/') + ".java")
                ...
                val s = renderTemplate("itemClassTemplate.java", mapOf(
                    ...
                    "items" to items,
                    "annotations" to it.classAnnotations,
                    "packageName" to it.classPath.substringBeforeLast('.'),
                    "className" to it.classPath.substringAfterLast('.')
                ))
    ...
    private fun writeBlocks(ctx: BaseContext) {
        for (cfg in ctx.config.blockConfigs) {
            val rawBlocks = ConfigFactory.parseFile(File(ctx.rootDir, cfg.file))
            val blockBase = rawBlocks.getConfigOrNull("_base")
            val blocks = rawBlocks
                .root()
                .filter { !it.key.startsWith("_") }
                .map { (id, elem) ->
                    val obj = (elem as ConfigObject).toConfig().let {
                        if (blockBase != null) it.withFallback(blockBase) else it
                    }
            ...
            run {
                val blocksClassFile = File(ctx.srcDir, cfg.classPath.replace('.', '/') + ".java")
                ...
                val s = renderTemplate("blockClassTemplate.java", mapOf(
                    ...
                    "annotations" to cfg.classAnnotations,
                    "packageName" to cfg.classPath.substringBeforeLast('.'),
                    "className" to cfg.classPath.substringAfterLast('.')
                ))
